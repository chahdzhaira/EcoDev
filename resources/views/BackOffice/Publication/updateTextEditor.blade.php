    ...
    <main class="app-content">
        <div class="col-md-12">
            <div class="tile">
                ...
                <div class="tile-body">
                <form action="/update/{{ $publication->id }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                      <label for="" style="font-weight: 300;">Title</label>
                      <input type="text" class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" name="title" value="{{  $publication->title }}" >
                      @error('title')
                    ...
                    <div class="mb-4">
                      ...
                      <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category" id="category">
                        <option value="Reducing waste" {{ $publication->category == 'Reducing waste' ? 'selected' : '' }}>Reducing waste</option>
                        <option value="Recycling" {{ $publication->category == 'Recycling' ? 'selected' : '' }}>Recycling</option>
                      </select>
                    </div>
                    <div class="mb-4">
                      <label for="" style="font-weight: 300;">Description</label>
                      <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" cols="30" rows="10">{{ $publication->description }}</textarea>
                      @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                    </div> 
